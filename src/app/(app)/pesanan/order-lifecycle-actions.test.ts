 ...
 * item-mutation module split. Covers createPesanan, getInvoiceData, and the
 * pesanan-row fields (status, shipping, delete). The guards these delegate to
 * are tested separately in lib/pesanan-guards.test.ts, so they're mocked
 * here — the point is to pin what the *actions* add on top:
 *
...
async function actions() {
  return import('./order-lifecycle-actions')
}
