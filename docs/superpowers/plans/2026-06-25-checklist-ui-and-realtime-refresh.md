...
#### Addendum: two real defects found during this step, both fixed

Live verification (driving the actual app in a browser against the real Supabase project, not just `npm run build`) surfaced two gaps that none of Tasks 4–7's automated checks could catch, since both are runtime Postgres/Realtime behaviors:

1. **`REPLICA IDENTITY DEFAULT` silently drops RLS-gated change events.** Supabase Realtime's `postgres_changes` evaluates each subscriber's RLS policy against the row image carried in the WAL. With the default replica identity (primary key only), Postgres doesn't include enough of the row for that RLS check on UPDATE/DELETE, so the event is silently dropped — the client's channel still reports `SUBSCRIBED`, making this failure mode invisible without actually triggering a change and watching for delivery. Fixed via a follow-up migration: `alter table public.pesanan/pelanggan/users replica identity full;` (recorded as `supabase/migrations/20260625093039_realtime_replica_identity_full.sql`).
2. **`@supabase/ssr`'s cookie-based browser client doesn't auto-propagate the JWT to the Realtime socket.** The socket initially joins as `anon`, so any RLS policy requiring an authenticated role (e.g. `pesanan`'s `current_user_role() IS NOT NULL`) filters out every event. Fixed in `src/hooks/use-realtime-refresh.ts` by explicitly calling `supabase.auth.getSession()` then `supabase.realtime.setAuth(session.access_token)` before `.channel(...).subscribe()`, plus an `onAuthStateChange` listener that re-applies `setAuth` on `TOKEN_REFRESHED` so long-lived page views don't go stale after the access token rotates. `src/hooks/use-realtime-refresh.test.ts` was updated to mock `auth.getSession`/`auth.onAuthStateChange`/`realtime.setAuth` and to await a microtask flush before asserting (channel creation now happens inside a `.then()`).

Both fixes were verified live: toggling `item_pesanan.diambil_oleh_helper` via direct SQL while a browser session had `/pesanan/[id]` open caused that open tab to update from "0/1 diambil" to "1/1 diambil" with no manual reload; inserting a `pelanggan` row via SQL while `/pelanggan` was open updated "0 pelanggan terdaftar" to "1 pelanggan terdaftar" with no manual reload.
